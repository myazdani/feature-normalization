...
def copmute_normalization(csv_row):
    if csv_row[0] in images: normalized_features.append(probability_normalize(row))
'''
...
for in_file in in_files:
    csv_f = csv.reader(open(in_file))
    #[copmute_normalization(row) for row in csv_f]
    Parallel(n_jobs = 2)(delayed(copmute_normalization)(row) for row in csv_f)
